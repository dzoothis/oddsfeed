...
class MatchesController extends Controller
{
    ...
    public function getMatches(Request $request)
    {
        try {
            ...
            if (!empty($matches)) {
                ...
                $matches = $this->enrichMatchesWithCacheData($matches, $sportId, $leagueIds, $matchType);
                $matches = $this->attachOddsFromCache($matches);

                // Check if database data might be stale and trigger background refresh
                $this->triggerBackgroundRefreshIfNeeded($sportId, $leagueIds, $matchType, $matches);
            ...
            $matches = $this->getMatchesFromCache($sportId, $leagueIds, $matchType);
    ...
    private function checkSystemHealth()
    {
        ...
        try {
            ...
            $staleMatches = \DB::table('sports_matches')
                ->where('lastUpdated', '<', now()->subHours(6))
                ->count();

            $totalMatches = \DB::table('sports_matches')->count();
