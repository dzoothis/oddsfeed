          ...
          <div class="match-card-header p-4 border-b border-gray-200">
            ...
            <div class="flex items-center justify-between mb-3">
              ...
              <div class="team-info flex items-center space-x-3 flex-1">
                <div class="team-logo">
                  <img v-if="match.images?.home_team_logo" :src="match.images.home_team_logo" :alt="match.home_team"
                    class="w-8 h-8 rounded-full object-cover border border-gray-200" @error="handleImageError">
                  <div v-else
                    class="w-8 h-8 rounded-full bg-gray-200 flex items-center justify-center text-xs font-bold text-gray-600 border border-gray-200">
                    {{ getTeamInitials(match.home_team) }}
                  </div>
                </div>
                <div class="team-details">
                  <div class="team-name font-semibold text-gray-900 text-lg">{{ match.home_team }}</div>
                  <div v-if="match.score && match.betting_availability === 'live'"
                    class="score-display text-xl font-bold text-blue-600">
                    {{ match.score.home }}
                  </div>
              ...
              <div class="match-vs px-4">
                <div class="text-center">
                  ...
                  <span :class="['inline-flex items-center px-2 py-1 text-xs font-medium rounded-full',
                    ...
                    {{ match.betting_availability === 'live' ? 'LIVE' : 'PREMATCH' }}
                  </span>
                </div>
              </div>
              ...
              <div class="team-info flex items-center space-x-3 flex-1 justify-end">
                ...
                <div class="team-logo">
                  <img v-if="match.images?.away_team_logo" :src="match.images.away_team_logo" :alt="match.away_team"
                    class="w-8 h-8 rounded-full object-cover border border-gray-200" @error="handleImageError">
                  <div v-else
                    class="w-8 h-8 rounded-full bg-gray-200 flex items-center justify-center text-xs font-bold text-gray-600 border border-gray-200">
            ...
            <div class="flex items-center justify-between">
              <div class="match-meta flex items-center space-x-4 text-sm text-gray-600">
                ...
                <span
                  v-if="match.betting_availability !== 'live' && match.scheduled_time && match.scheduled_time !== 'TBD'"
                  class="match-datetime px-3 py-1 bg-blue-100 text-blue-800 text-sm rounded-lg font-semibold">
                  {{ formatMatchDateTime(match) }}
                </span>
                <span v-if="match.home_team_data?.country"
                  class="match-country px-2 py-0.5 bg-gray-100 text-gray-700 text-xs rounded">
                  {{ match.home_team_data.country }}
                </span>
                <span v-if="match.duration && match.betting_availability === 'live'"
                  class="match-duration px-2 py-0.5 bg-orange-100 text-orange-700 text-xs rounded font-medium">
                  {{ match.duration }}
                </span>
              </div>
              ...
              <div class="match-status-group flex items-center space-x-3">
                ...
                <!--Uncomment -->
                <span v-if="match.last_updated"
                  class="updated-badge px-2 py-1 bg-green-50 text-green-700 text-xs rounded-md font-medium border border-green-200">
                  Updated {{ formatTimeAgo(match.last_updated) }} ago
                </span>

                <!-- Markets Badge -->
                <div v-if="match.has_open_markets || match.odds_count > 0"
                  class="markets-badge flex items-center space-x-1">
                ...
                <!-- Expand Button -->
                <button type="button" @click.prevent.stop="toggleMatchExpansion(match.id)" :class="['expand-button p-2 rounded-lg hover:bg-gray-100 transition-colors',
                  expandedMatches.includes(match.id) ? 'bg-blue-50 text-blue-600' : 'text-gray-500']">
                  <svg class="w-4 h-4 transition-transform duration-200"
                    :class="{ 'rotate-180': expandedMatches.includes(match.id) }" fill="none" stroke="currentColor"
                    viewBox="0 0 24 24">
          ...
          <div v-if="expandedMatches.includes(match.id)" class="match-card-body border-t border-gray-200">
            ...
            <div class="market-tabs">
              <div class="market-tabs-header flex border-b border-gray-200">
                <button v-for="market in availableMarkets" :key="market.key" @click="selectMarket(match.id, market.key)"
                  :disabled="loadingMarkets[match.id]" :class="['market-tab px-4 py-2 text-sm font-medium border-b-2',
              ...
              <div class="market-tabs-content p-4">
                  ...
                  <div class="flex items-center justify-between mb-3">
                    <div class="flex items-center gap-3 flex-1">
                      <div class="flex items-center gap-2 flex-wrap">
                        <!-- Period Filter -->
                        <div class="relative">
                          <button @click="toggleDropdown(match.id, 'period')" type="button"
                            ...
                            <svg class="w-4 h-4 transition-transform"
                              :class="{ 'rotate-180': dropdowns[match.id]?.period }" fill="none" stroke="currentColor"
                              viewBox="0 0 24 24">
                        ...
                        <!-- Status Filter -->
                        <div class="relative">
                          <button @click="toggleDropdown(match.id, 'status')" type="button"
                            class="px-3 py-2 text-sm bg-white border border-gray-300 rounded-lg hover:bg-gray-50 flex items-center gap-2 min-w-[120px]">
                            <span>{{ getFilterValue(match.id, 'status') || 'Status' }}</span>
                            <svg class="w-4 h-4 transition-transform"
                              :class="{ 'rotate-180': dropdowns[match.id]?.status }" fill="none" stroke="currentColor"
                              viewBox="0 0 24 24">
                              <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7">
                              </path>
                            </svg>
                ...
                <div v-if="marketOdds[match.id] && marketOdds[match.id][activeMarket(match.id)]" class="market-table">
                  <div class="mb-3 text-sm text-gray-600">
                    Showing {{ marketOdds[match.id][activeMarket(match.id)].showing_count }} of {{
                      marketOdds[match.id][activeMarket(match.id)].total_count }} markets
                  </div>
                    ...
                    <table class="w-full">
                      <thead>
                        <tr class="market-table-header bg-gray-50">
                          <th
                            class="market-table-th px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">
                            Bet</th>
                          <th
                            class="market-table-th px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">
                            Line</th>
                      ...
                      <tbody class="bg-white divide-y divide-gray-200">
                        <tr v-for="odd in filteredOdds(match.id)" :key="odd.id || Math.random()"
                          ...
                          <td class="market-table-td px-4 py-3">
                            ...
                            <!-- Uncomment -->
                            <span v-if="odd.updated_at"
                              class="ml-1.5 px-1.5 py-0.5 bg-green-100 text-green-800 text-xs rounded font-medium">
...
const getTeamInitials = (teamName) => {
  ...
  // Remove parentheses and their contents (e.g., "Club Tijuana (Corners)" -> "Club Tijuana")
  let cleanName = teamName.replace(/\s*\([^)]*\)/g, '').trim();
  ...
  if (words.length === 0) {
    ...
    const originalWords = teamName.split(' ').filter(word => word.length > 0);
    if (originalWords.length === 1) {
      return originalWords[0].substring(0, 2).toUpperCase();
    }
    return (originalWords[0][0] + originalWords[originalWords.length - 1][0]).toUpperCase();
  }
